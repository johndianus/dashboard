...
const columns = [
  { key: 'name', label: 'Name', width: '35%', bold: false, align: 'left' },
  { key: 'organization_name', label: 'Organization', width: '35%', bold: false, align: 'left' },
  { key: 'job_title', label: 'Job Title', width: '30%', bold: false, align: 'left' },
]
...
const Customers = () => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const masterData = await fetchJson(
          `${baseURL}contacts?db=${storedCompanyname}&user=1&page=${currentPage}&limit=${itemsPerPage}`,
        )
        const masters = masterData?.data ?? []
        setData(restructureCustomers(masters))
  ...
  const editCustomer = (item) => {
    navigate(`/masters/customer?customerid=${item.id}`)
  }

  console.log(paginatedData)
  return (
    <CCard className="shadow-sm rounded-4">
      <CCardHeader className="bg-light fw-semibold text-primary fs-5">List of Contacts</CCardHeader>
      <CCardBody>
