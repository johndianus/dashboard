...
const columns = [
  { key: 'name', label: 'Name', width: '25%', bold: false, align: 'left' },
  { key: 'description', label: 'Description', width: '35%', bold: false, align: 'left' },
]
...
const Services = () => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const masterData = await fetchJson(
          `${baseURL}masters?db=${storedCompanyname}&object=service_types&user=1&page=${currentPage}&limit=${itemsPerPage}`,
        )
  ...
  const handleAddContact = () => {
    navigate('/masters/service')
  }

  const editCustomer = (item) => {
    navigate(`/masters/service?serviceid=${item.service_type_id}`)
  }

  console.log(paginatedData)
  return (
    <CCard className="shadow-sm rounded-4">
      <CCardHeader className="bg-light fw-semibold text-primary fs-5">List of Services</CCardHeader>
      <CCardBody>
