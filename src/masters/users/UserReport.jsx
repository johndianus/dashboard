...
const columns = [
  { key: 'username', label: 'Username', width: '25%', bold: false, align: 'left' },
  { key: 'email', label: 'Email', width: '25%', bold: false, align: 'left' },
  { key: 'displayname', label: 'Display Name', width: '25%', bold: false, align: 'left' },
  { key: 'rolename', label: 'Role Name', width: '25%', bold: false, align: 'left' },
]
...
const Services = () => {
    ...
    const fetchAll = async () => {
      try {
        ...
        const masterData = await fetchJson(
          `${baseURL}users?db=${storedCompanyname}&user=1&page=${currentPage}&limit=${itemsPerPage}`,
        )
  ...
  const editCustomer = (item) => {
    navigate(`/masters/user?userid=${item.id}`)
  }

  console.log(paginatedData)
  return (
    <CCard className="shadow-sm rounded-4">
      <CCardHeader className="bg-light fw-semibold text-primary fs-5">List of Users</CCardHeader>
      <CCardBody>
